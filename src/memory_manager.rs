//! Primitive static memory allocator.
//!
//! This can be used as a wrapper over a static chunk of memory to let
//! components of the application requests parts of it.

use core::fmt;
use core::mem::MaybeUninit;

/// Memory manager redistributing given chunk of memory.
///
/// # Examples
///
/// Using static slice:
...
/// ```
///
/// Using passed pointer, for example mounted for DMA to an external RAM:
///
/// ```
/// use core::mem::MaybeUninit;
/// use sirena::memory_manager::MemoryManager;
...
/// let ram_base_address = fake_ram.as_mut_ptr();
///
/// let ram_slice = unsafe {
///     let ram_items = ram_size / core::mem::size_of::<MaybeUninit<u32>>();
///     let ram_ptr = ram_base_address as *mut MaybeUninit<u32>;
///     core::slice::from_raw_parts_mut(ram_ptr, ram_items)
/// };
/// let mut memory_manager = MemoryManager::from(ram_slice);
///
/// let slice = memory_manager.allocate(5).unwrap();
/// assert_eq!(slice.len(), 5);
///
...
/// # Safety
///
/// Passed memory must never get deallocated or accessed through other means.
